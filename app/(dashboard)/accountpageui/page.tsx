...
try {
const res = await fetch(baseUrl + `/customers/update/${form.email}`, {
method: "PUT",
headers: { "Content-Type": "application/json" },
...
try {
const res = await fetch(baseUrl + `/customers/delete/${identificationNumber}`, {
method: "DELETE",
headers: { "Content-Type": "application/json" },
});
const json = await res.json();
...
return ( <div className="max-w-6xl mx-auto p-6 space-y-6"> <h1 className="text-3xl font-bold">Customers</h1>
  ...
  <table className="w-full border border-gray-300 rounded-lg overflow-hidden">
    ...
    <tbody>
      {displayedUsers.map((user) => (
        <tr key={user.identificationNumber} className="border-t border-gray-200 hover:bg-gray-50">
          ...
          <td className="p-3 flex justify-center gap-2">
            <button onClick={() => openEdit(user)} className="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600">Edit</button>
            <button onClick={() => handleDelete(user.identificationNumber)} className="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600">Delete</button>
          </td>
        </tr>
      ))}
      {displayedUsers.length === 0 && (
        <tr>
          <td colSpan={7} className="text-center p-4 text-gray-500">No customers found.</td>
        </tr>
      )}
    </tbody>
  </table>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-xl shadow-xl w-full max-w-xl p-6 z-10">
        ...
        <div className="grid grid-cols-1 gap-3">
          {["firstName","middleName","lastName","email","address","contact"].map((field) => (
            <label key={field} className="flex flex-col">
              <span className="text-sm font-medium text-gray-600">{field.charAt(0).toUpperCase() + field.slice(1).replace(/([A-Z])/g, ' $1')}</span>
              <input
                name={field}
        ...
        <div className="flex justify-end gap-3 mt-4">
          ...
          <button onClick={() => handleSave(editingUser.identificationNumber)} className="px-4 py-2 rounded bg-blue-600 text-white">Save Changes</button>
        </div>
      </div>
    </div>
  )}
